...
`relayos` includes the engine, framework adapters (`relayos/next-js`, `relayos/express`, `relayos/hono`, `relayos/nestjs`), and the `relay` CLI - this guide adds a database as you go. No separate CLI install, ever - see [CLI](/docs/cli) for the full command reference, including a zero-install `npx relayos@latest init` path if you don't even have a project yet.
...
Migrations still apply automatically outside production, but in production run them explicitly before scaling up new instances. The `relay` command comes bundled with `relayos` from step 1 - nothing else to install:
...
Already have `relayos` in your project? `relay` is already available:
