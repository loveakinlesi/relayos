...
| `--forward <url>` | No       | URL to POST to. Default is `RESTAQ_BASE_URL` or `http://localhost:3000`                       |
...
| `RESTAQ_BASE_URL`       | No `--forward` specified    | Where to POST the webhook. Defaults to `http://localhost:3000` |
