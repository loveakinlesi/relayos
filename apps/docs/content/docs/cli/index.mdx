...
The `relay` command comes bundled with `relayos` - nothing to install separately. Since `relayos` is already the one required dependency of any RelayOS project, the CLI is available the moment you install it.
...
This fetches `relayos` (and its bundled CLI) ephemerally, runs the [interactive wizard](/docs/cli/init), and scaffolds `relay.ts` + `relay.handlers.ts` - installing `relayos` for real along the way, so every command after this one runs locally.

## Already have `relayos` installed?

The `relay` bin is available immediately, no separate `@relayos/cli` install needed:
...
Want the CLI as its own dependency instead (e.g. a CI image that only runs `relay migrate`, not the full app)? `@relayos/cli` is still published standalone and provides the identical `relay` bin:
...
Don't install both in the same project - they provide the same `relay` bin name, and only one will win the symlink.
