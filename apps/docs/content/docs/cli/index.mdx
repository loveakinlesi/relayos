---
title: CLI
description: Manage Restaq from the command line with migrate, dev, trigger, inspect, replay, and more.
---
...
This fetches `restaq` (and its bundled CLI) ephemerally, runs the [interactive wizard](/docs/cli/init), and scaffolds `relay.ts` + `relay.handlers.ts` — installing `restaq` for real along the way, so every command after this one runs locally.
...
<Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
  ...
  <Tab value="npm">
  ```sh
  npm install -D @restaq/cli
  ```
  </Tab>
  <Tab value="yarn">
  ```sh
  yarn add -D @restaq/cli
  ```
  </Tab>
  <Tab value="bun">
  ```sh
  bun add -d @restaq/cli
  ```
  </Tab>
</Tabs>
