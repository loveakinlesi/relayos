...
|------|---------|-------------|
| `--forward <url>` | `RELAYOS_BASE_URL` or `http://localhost:3000` | URL to POST the replay to |
| `--print` | false | Dry-run: show what would be replayed without POSTing |
| `--dir` | current directory | Directory containing `relay.ts`, used to load the historical execution |

## Requirements

- A `relay.ts` in the target directory, exporting `relay` (to load the historical execution)
...
```sh
RELAYOS_BASE_URL=https://staging.example.com relay replay evt_abc123
```
