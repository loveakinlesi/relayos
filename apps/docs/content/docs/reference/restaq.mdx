...
Creates a Restaq runtime instance. This is the main entry point for developers: register event handlers via `relay.on()`, receive provider webhooks through `relay.handler`, and configure persistence via `config.database` — a Postgres/SQLite/MySQL client (auto-detected), a `{ dialect }` Kysely escape hatch, or a custom `ExecutionStore`. The relay's event types are inferred from the plugins you register—typed plugins make `relay.on()` autocomplete event names and narrow event payloads.
