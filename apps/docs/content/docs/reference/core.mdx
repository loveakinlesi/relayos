...
| `createMemoryExecutionStore` | Function | Creates an in-memory execution store (default if no database is configured). |
...
```ts
type EventHandler<
  TEvent extends NormalizedEvent<string, any> = NormalizedEvent,
> = (event: TEvent, ctx: RuntimeContext) => void | Promise<void>;
```
...
The relay runtime. Typed `on()` overloads narrow event types and payloads based on registered plugins. `ingest()` is the low-level event entry point (plugins and adapters call this). Execution replay, retry, and restart methods allow recovery from failures.
...
Configuration for `createRelayEngine()`. Defaults to an in-memory store and 3 retries with exponential backoff.
...
```ts
function createMemoryExecutionStore(): ExecutionStore;
```

Creates a transient in-memory store. Useful for testing or short-lived processes. Lost on restart.
