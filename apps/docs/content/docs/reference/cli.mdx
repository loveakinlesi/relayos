---
title: '@restaq/cli'
description: CLI for Restaq migrations, development, and execution inspection.
---

`restaq` bundles this package's `relay` bin automatically — most projects
never need to install `@restaq/cli` directly. See [CLI](/docs/cli) for the
...
<Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
  ...
  <Tab value="npm">
  ```sh
  npm install -g @restaq/cli
  ```
  </Tab>
  <Tab value="yarn">
  ```sh
  yarn global add @restaq/cli
  ```
  </Tab>
  <Tab value="bun">
  ```sh
  bun add -g @restaq/cli
  ```
  </Tab>
</Tabs>
