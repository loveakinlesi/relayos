---
title: Resend Plugin
description: Receive and verify Resend webhooks with Restaq.
---

The `@restaq/resend` plugin handles [Resend webhook](https://resend.com/docs/dashboard/webhooks/introduction) verification and provides typed Restaq event names for Resend email events. Like Clerk, Resend delivers webhooks via [Svix](https://www.svix.com), so verification follows the same Svix signature scheme.
...
<Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
  ...
  <Tab value="npm">
  ```sh
  npm install @restaq/resend
  ```
  </Tab>
  <Tab value="yarn">
  ```sh
  yarn add @restaq/resend
  ```
  </Tab>
  <Tab value="bun">
  ```sh
  bun add @restaq/resend
  ```
  </Tab>
</Tabs>
