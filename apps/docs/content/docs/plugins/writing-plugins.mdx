...
const myPlugin: RelayPlugin = {
  id: 'myprovider', // Used as the URL segment: /api/relay/myprovider
...
// Now when you pass myPlugin to createRelay, relay.on() autocompletes the event names
...
Here's the test plugin from `apps/web/relayos.config.ts` — it never verifies signatures, and forces all events under the `test.*` namespace so it can't spoof real providers:
