...
Here's the test plugin from `apps/web/relay.ts` — it never verifies signatures, and forces all events under the `test.*` namespace so it can't spoof real providers:
