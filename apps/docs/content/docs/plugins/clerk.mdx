...
The `@relayos/clerk` plugin handles [Clerk webhook](https://clerk.com/docs/webhooks/overview) verification and provides typed RelayOS event names for common Clerk webhook events. Clerk delivers webhooks via [Svix](https://www.svix.com), so verification follows the Svix signature scheme.
