...
Receiving a webhook is easy. Reliably processing everything that has to happen _after_ it — retrying the parts that failed, not re-running the parts that didn't, replaying a historical event to debug it, seeing exactly what happened and why — is the hard part. Restaq is the runtime for that.
...
| Package                                  | Purpose                                                                                                                                                                                                                                                                                                                                                                                          |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [`restaq`](./packages/restaq)          | The SDK: `restaq`, the user-facing types, and framework adapters (`restaq/next-js`, `restaq/express`, `restaq/hono`, `restaq/nestjs`). The only required install.                                                                                                                                                                                                                           |
| [`@restaq/core`](./packages/core)       | The engine: durable executions, steps, retries, replay, and the contracts (`ExecutionStore`, `RelayPlugin`) everything else builds on. Also owns database support — pass a raw Postgres/SQLite/MySQL client into `restaq({ database })` and the dialect is auto-detected (built on [Kysely](https://kysely.dev), with hand-written versioned migrations). Pulled in automatically by `restaq`. |
| [`@restaq/plugin`](./packages/plugin)   | Plugin authoring kit: `definePlugin` + webhook signature helpers for building your own providers. Optional.                                                                                                                                                                                                                                                                                      |
| [`@restaq/stripe`](./packages/stripe)   | Stripe plugin: signature verification + a fully-typed catalog of every Stripe event (peer-depends on `stripe`). Optional.                                                                                                                                                                                                                                                                        |
| [`@restaq/github`](./packages/github)   | GitHub plugin: signature verification + a fully-typed catalog of every GitHub webhook event. Optional.                                                                                                                                                                                                                                                                                           |
| [`@restaq/clerk`](./packages/clerk)     | Clerk plugin: Svix signature verification + typed event names for common Clerk webhook events. Optional.                                                                                                                                                                                                                                                                                         |
| [`@restaq/shopify`](./packages/shopify) | Shopify plugin: HMAC signature verification + typed event names for common Shopify webhook topics. Optional.                                                                                                                                                                                                                                                                                     |
| [`@restaq/resend`](./packages/resend)   | Resend plugin: Svix signature verification + typed event names for Resend email events. Optional.                                                                                                                                                                                                                                                                                                |
| [`@restaq/cli`](./packages/cli)         | `relay init`, `relay migrate`, `relay dev`, `relay trigger`, `relay inspect`, `relay replay`, `relay events list`.                                                                                                                                                                                                                                                                               |
...
- Node >=22, pnpm >=9
- `@restaq/core`'s database contract tests want a container runtime (Docker, Colima, or Podman) for the Postgres/MySQL legs — see [Testing](#testing) below if you don't have one.
